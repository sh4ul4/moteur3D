...
namespace M3D_ISICG
{
	...
	bool LabWork4::init()
	{
		...
		_conference.load( "conference", "./conference/conference.obj" );

		_conference._transformation = glm::scale( _conference._transformation, glm::vec3( 0.01, 0.01, 0.01 ) );
	...
	void LabWork4::render()
	{
		...
		_camera._computeUMVPMatrix( _conference._transformation );
		updateuMVPmatrix();

		_camera._computeUMVMatrix( _conference._transformation );
		updateuMVMatrix();
		updateuMMatrix();
		updateuNormalMatrix();

		_conference.render( _program );
	}

	void LabWork4::updateuMVPmatrix() {
		glProgramUniformMatrix4fv( _program, _uMVPMatrixLoc, 1, false, glm::value_ptr( _camera.getUMVPMatrix() ) );
	}

	void LabWork4::updateuMVMatrix()
	{
		glProgramUniformMatrix4fv( _program, _uMVMatrixLoc, 1, false, glm::value_ptr( _camera.getUMVMatrix() ) );
	}

	void LabWork4::updateuMMatrix()
	{
		glProgramUniformMatrix4fv( _program, _uMMatrixLoc, 1, false, glm::value_ptr( _conference._transformation ) );
	}

	void LabWork4::updateuNormalMatrix() {
		glProgramUniformMatrix4fv( _program, _uNormalMatrixLoc, 1, false, glm::value_ptr( glm::transpose( glm::inverse( _camera.getUMVMatrix() ) ) ) );
	}
